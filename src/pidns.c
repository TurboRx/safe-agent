...
{
    /* security boundary: direct unshare clone_newpid succeeds if running as root or with cap_sys_admin */
    ...
    /* security boundary: unprivileged pid namespace creation requires user namespace */
    if (unshare(CLONE_NEWUSER | CLONE_NEWPID) < 0) {
        fprintf(stderr, "safe-agent: failed to unshare pid namespace: %s\n", strerror(errno));
        return -1;
    }
